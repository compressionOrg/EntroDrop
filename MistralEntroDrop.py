from transformers.models.mistral.modeling_mistral import MistralForCausalLM, MistralModel, MistralDecoderLayer, MistralConfig
...
class MistralDecoderLayerDrop(MistralDecoderLayer):
    ...
    def estimate_entropy_knn_fixed(self, data, k=5):
        """
        修正后的kNN熵估计，使用对数伽马函数避免数值溢出。
        """
        batch_size, sequence_length, hidden_dim = data.shape
        flattened_data = data.reshape(-1, hidden_dim)

        nbrs = NearestNeighbors(n_neighbors=k + 1, algorithm='auto').fit(flattened_data)
        distances, _ = nbrs.kneighbors(flattened_data)
        radii = distances[:, 1:]

        n = flattened_data.shape[0]
        d = hidden_dim
        avg_log_radius = np.mean(np.log(radii[:, -1] + 1e-10))  # 防止log(0)
        
        # 使用对数伽马函数避免溢出
        entropy = (d * avg_log_radius) + (d / 2) * np.log(np.pi) - gammaln(d / 2 + 1) + np.log(n) - np.log(k)
        return entropy

    def compute_information_gain_fixed(self, X, Y, k=50):
        """
    ...
    def compute_information_gain_marginal_binning(self, X, Y, num_bins=80):
        """
    ...
    def compute_cosine_importance(self, X, Y):
        ...
        importance = 1 - torch.mean(cos_sim)
        return importance.item(), 0
    ...
    def forward(
        ...
        **kwargs,
        ...
        score_attn, entropy_y_d_attn = self.compute_information_gain_marginal_binning(residual, hidden_states)
        ...
        hidden_states = residual + hidden_states
        score_layer, _ = self.compute_information_gain_marginal_binning(residual_orig, hidden_states)
...
class MistralModelDrop(MistralModel):
    # init the model 
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.drop_attentions = None
        self.layers = nn.ModuleList([MistralDecoderLayerDrop(config, i) for i in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            entropy_y_d_list.extend([entropy_y_d_attn, entropy_y_d_mlp])
            layer_drop_score_list.append(drop_score_layer)
...
class MistralForCausalLMDrop(MistralForCausalLM):
    # init the model 
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        config._attn_implementation = "eager"
        self.model = MistralModelDrop(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        is_prefilling_stage = self.is_prefilling_stage(input_ids)
        ...
        outputs = self.model(
            ...
            cache_position=cache_position,
            prefilling_state=is_prefilling_stage,
            **kwargs,
        )
