...
def get_calibration_dataloader(
    dataset_name: Literal["wikitext2", "ptb", "c4", "pile", 'boolq', 'openbookqa', 'arc_easy', 'arc_challenge', 'hellaswag', 'winogrande', 'piqa', 'mathqa'],
    tokenizer,
    ...
    if 'wikitext2' in dataset_name:
        ...
        # train_data = load_dataset('monology/pile-uncopyrighted', split='train[:5000]')
        # traindata = load_dataset("monology/pile-uncopyrighted", data_files={"train": "val.jsonl.zst"})
        train_data = load_from_disk('datasets/pile/val')['train']
        random_indices = random.sample(range(len(train_data)), num_samples)
        train_data = train_data.select(random_indices)
        train_dataset = process_pretrain_data(train_data, tokenizer, seq_len, 'text')
        data_collator = None
    elif 'redpajama' in dataset_name:
        train_data = load_dataset('togethercomputer/RedPajama-Data-V2', 'sample', split='train[:1000]')
        random_indices = random.sample(range(len(train_data)), num_samples)
        train_data = train_data.select(random_indices)
        train_dataset = process_pretrain_data(train_data, tokenizer, seq_len, 'raw_content')
        data_collator = None
    elif 'oscar' in dataset_name:
        train_data = load_dataset('oscar', 'unshuffled_deduplicated_en', split='train[:5000]')
        random_indices = random.sample(range(len(train_data)), num_samples)
        train_data = train_data.select(random_indices)
        train_dataset = process_pretrain_data(train_data, tokenizer, seq_len, 'raw_content')
        data_collator = None
    elif "slimpajama" in dataset_name:
        # train_data = load_dataset('DKYoon/SlimPajama-6B')['train']
        train_data = load_from_disk("datasets/SlimPajama-6B/train")
        random_indices = random.sample(range(len(train_data)), num_samples)
        train_data = train_data.select(random_indices)
        train_dataset = process_pretrain_data(train_data, tokenizer, seq_len, 'text')
        data_collator = None
...
def get_evaluation_dataloader(dataset_name: Literal["wikitext2", "ptb", "c4"], tokenizer):
    ...
    if "pile" in dataset_name:
        testdata = load_dataset('monology/pile-uncopyrighted', split='train[5000:5100]', verification_mode='no_checks')
        testenc = tokenizer("\n\n".join(testdata["text"]), return_tensors="pt")
        return testenc
    if "redpajama" in dataset_name:
        testdata = load_dataset('togethercomputer/RedPajama-Data-V2', 'sample', split='test')
        testenc = tokenizer("\n\n".join(testdata["raw_content"]), return_tensors="pt")
        return testenc
    if "oscar" in dataset_name:
        testdata = load_dataset('oscar', 'unshuffled_deduplicated_en', split='train[5000:10000]')
        testenc = tokenizer("\n\n".join(testdata["text"]), return_tensors="pt")
        return testenc
    if "slimpajama" in dataset_name:
        # testdata = load_dataset('DKYoon/SlimPajama-6B')['test']
        testdata = load_from_disk("datasets/SlimPajama-6B/test")
        testenc = tokenizer("\n\n".join(testdata["text"]), return_tensors="pt")
        return testenc
    raise NotImplementedError
...
def get_test_data(name, tokenizer, seq_len=2048, batch_size = 4):
    ...
    if 'wikitext2' in name:
        ...
        test_data = load_dataset('monology/pile-uncopyrighted', split='train[5000:5100]', verification_mode='no_checks')
        test_dataset = process_data(test_data[5000:5100], tokenizer, seq_len, 'text')
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    return test_loader

def get_train_data(name, tokenizer, seq_len=2048, batch_size = 4, seed=42):
    random.seed(seed)
    class IndexDataset(Dataset):
        def __init__(self, tensors):
            self.tensors = tensors
    ...
    if 'wikitext2' in name:
        # test_data = load_dataset('wikitext', 'wikitext-2-raw-v1', split='test')
        train_data = load_from_disk("datasets/wikitext/train")
        train_dataset = process_data(train_data, tokenizer, seq_len, 'text')
    elif 'ptb' in name:
        # test_data = load_dataset('ptb_text_only', 'penn_treebank', split='test')
        train_data = load_from_disk("datasets/ptb/train")
        train_dataset = process_data(train_data, tokenizer, seq_len, 'sentence')
    elif 'c4' in name:
        train_data = load_from_disk("datasets/c4/train")
        train_dataset = process_data(train_data, tokenizer, seq_len, 'text')
    elif 'pile' in name:
        train_data = load_dataset('monology/pile-uncopyrighted', split='train[:5000]', verification_mode='no_checks')
        train_dataset = process_data(train_data, tokenizer, seq_len, 'text')
    elif "slimpajama" in dataset_name:
        train_data = load_from_disk("datasets/SlimPajama-6B/train")
        train_dataset = process_data(train_data, tokenizer, seq_len, 'text')
