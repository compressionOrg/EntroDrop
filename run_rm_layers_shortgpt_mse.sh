...
log_file="llama3.1_8b_shortgpt_mse_count_param.log"
# 循环执行不同的 num_prune 值
for num_prune in 4 6 8 10 12; do
    echo "Running with num_prune=$num_prune"
    python run_rm_layers.py --model_name $MODEL_NAME --layers_order  ${layers_order}  --num_prune $num_prune  --log_file $log_file
